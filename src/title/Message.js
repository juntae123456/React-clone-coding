...
const Message = () => {
  ...
  // 처음에 사용자 목록 가져오기
  useEffect(() => {
    fetch('http://10.0.1.38:3001/getusers') // 사용자 목록 가져오는 API 호출
      .then((response) => response.json())
      .then((data) => {
        // 로그인한 사용자 제외한 목록 필터링
          ...
          <List>
            {users.map((user) => (
              <ListItem button key={user.id} onClick={() => handleUserSelect(user)}>
                ...
                {user.newMessages && user.newMessages > 0 && (
                  <Badge badgeContent={user.newMessages} color="error" />
                )}
              </ListItem>
            ))}
          </List>
