...
function StoryList() {
  ...
  const fetchUsersWithStories = () => {
    fetch('http://localhost:3001/getusers') // 스토리가 있는 사용자 목록 API 호출
      ...
      <Swiper spaceBetween={20} slidesPerView="auto"> {/* 사용자 간 간격 설정 */}
        {users.map((user) => (
          <SwiperSlide key={user.id} style={{ width: 'auto' }}>
            <Box mx={1} textAlign="center" display="flex" flexDirection="column" alignItems="center"> {/* 텍스트 및 프로필 중앙 정렬 */} {/* 텍스트 및 프로필 중앙 정렬 */}
              <Avatar
                sx={{
              ...
              >
                {!user.profileImage && user.name[0].toUpperCase()} {/* 프로필 이미지가 없을 경우 이름 첫 글자 표시 */}
              </Avatar>
              <Typography variant="caption" sx={{ textAlign: 'center', mt: 1 }}> {/* 텍스트 중앙 정렬 및 위와 간격 조정 */}
                {user.name}
